--14 Вывести список прикреплений, для которых стоимость годового прикрепления (CLHISTNUM.AMOUNTRUB) меньше половины стоимости фактически оказанных по этому прикреплению услуг (TREAT.AMOUNTJP).
--Указать ФИО пациента, номер договора (JPAGREEMENT.AGNUM), номер полиса (CLHISTNUM.NSP), период прикрепления (CLHISTNUM.BDATE,FDATE)
set term ~;
create or alter procedure P_TEST_14 (YEARVAL int)
returns(
